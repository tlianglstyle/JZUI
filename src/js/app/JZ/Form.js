...
exports.InitEventKeyUp = function(opts){
	$('form[data-submit]').each(function(){
		var _this = $(this);
		var _btn = _this.find(_this.data('submit'));
		_this.attr("tabindex",0).keydown(function(event){
		  if(event.keyCode ==13){
		    _btn.trigger("click");  
		  }  
		});
		...
		if(_this.find('[type=submit]').length == 0){
			_this.find('input[type=text],input[type=password],textarea').keyup(function(event){  
			  if(event.keyCode ==13){
			    _btn.trigger("click");  
			  }  
			});
